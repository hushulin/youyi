...
class PositionAction extends AdminAction {
	...
	public function index(){
		...
		$Position = D("Position");  // 实例化Position对象
		import("ORG.Util.Page");                    // 导入分页类
		$count      = $Position->where($map)->count();   // 查询满足要求的总记录数
		...
		$list = $Position->where($map)->limit($Page->firstRow.','.$Page->listRows)->order("id desc")->select();
	...
	public function add(){
		...
		if(IS_POST){
			if (!$Position->create()){
				...
				// 验证通过 可以进行其他数据操作
				$re = $Position->addPosition();
				if ($re !== false) {
					$rurl = base64_decode($_GET["rurl"]);
					$rurl = empty($rurl)?"/Admin/Position":$rurl;
	...
	public function del(){
		...
		if(IS_POST){
			...
			$map = array();
			$map["id"] = $id;
			$re = $Position->where($map)->deletePosition();
			if ($re !== false) {
				$rurl = base64_decode($_GET["rurl"]);
				$rurl = empty($rurl)?"/Admin/Position":$rurl;
