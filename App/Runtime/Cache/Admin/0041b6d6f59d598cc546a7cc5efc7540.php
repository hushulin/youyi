...
<body onselectstart="return false">
	...
	<div id="main" class="g-main">
		...
		<div class="row">
			<div class="left">
				<a class="u-btn" href="<?php echo U('/Admin/Article/add');?>?rurl=<?php echo base64_encode($rurl);?>">新 增</a>
			</div>
						...
						<div>
							...
							<div class="ipt">
								<select name="key" class="u-select" style="width:80px;">
									<option value="title" <?php if($kkey == "title") echo 'selected="selected"'; ?>>标题</option><option value="content" <?php if($kkey == "content") echo 'selected="selected"'; ?>>正文</option>								</select>
								<input name="value" type="text" class="u-ipt" value="<?php echo ($value); ?>"/>
		...
		<div class="row">
			<form id="main_form" action="<?php echo U('/Article/maction');?>" method="get">
				<table class="m-table">
					<thead>
						<tr>
					...
					<tbody>
						<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
							...
							<td><?php if(isset($field_title)){echo $field_title[$vo["title"]];}else{echo $vo["title"];}?></td><td><?php if(isset($field_cate_id)){echo $field_cate_id[$vo["cate_id"]];}else{echo $vo["cate_id"];}?></td><td><?php if(isset($field_click)){echo $field_click[$vo["click"]];}else{echo $vo["click"];}?></td>							
							<td>
								<a href="<?php echo U('/Admin/Article/edit/id/'.$vo['id']);?>?rurl=<?php echo base64_encode($rurl);?>" title="">编辑</a>
								<a href="<?php echo U('/Admin/Article/del/id/'.$vo['id']);?>?rurl=<?php echo base64_encode($rurl);?>" title="">删除</a>
							</td>
