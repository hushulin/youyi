...
<body onselectstart="return false">
	...
	<div id="main" class="g-main">
		...
		<div class="row">
<?php  $rurl = base64_decode($_GET["rurl"]); $rurl = empty($rurl)?U("/Admin/Tipslist"):$rurl; ?>			<a class="u-btn" href="<?php echo ($rurl); ?>">返回列表</a>
		</div>
		<div class="row">
					...
					<fieldset>
						<div class="tab tab_1">
							...
							<div class="formitm">
									...
									<div class="ipt">
										<select class="u-select" name="position_id"><?php if(is_array($field_position_id)): $i = 0; $__LIST__ = $field_position_id;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($key); ?>" <?php if($key == $doc["position_id"]) echo 'selected="selected"'; ?> ><?php echo ($vo); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
</select>
										<p></p>
									</div>
								</div>
							<div class="formitm">
									...
									<div class="ipt">
										<select class="u-select" name="cargoods_id"><?php if(is_array($field_cargoods_id)): $i = 0; $__LIST__ = $field_cargoods_id;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($key); ?>" <?php if($key == $doc["cargoods_id"]) echo 'selected="selected"'; ?> ><?php echo ($vo); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
</select>
										<p></p>
									</div>
								</div>
							...
							<div class="formitm">
									...
									<div class="ipt">
										<input type="text" class="u-ipt time" name="time" value="<?php echo (date("Y-m-d h:i",$doc["time"])); ?>"/>
										<p></p>
									</div>
								</div>
							<div class="formitm">
					...
					KindEditor.ready(function(K) {
						editor = K.create('textarea[name="content"]', {
							...
							<div class="formitm">
									...
									<div class="ipt">
										<?php if(is_array($field_is_effect)): $i = 0; $__LIST__ = $field_is_effect;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><input type="radio" name="is_effect" value="<?php echo ($key); ?>" <?php if($key == $doc["is_effect"]) echo 'checked="checked"'; ?> >&nbsp;&nbsp;<?php echo ($vo); ?>&nbsp;&nbsp;&nbsp;&nbsp;<?php endforeach; endif; else: echo "" ;endif; ?>
										<p></p>
									</div>
								</div>
						</div>
