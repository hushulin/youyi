...
<<?php echo ($block); ?> name="body">
	...
	<div id="main" class="g-main">
		...
		<div class="row m-title">
			<h2><?php echo ($action_title); ?>管理 - 编辑<?php echo ($action_title); ?></h2>
		</div>
		<div class="row">
<?php echo ($phpstart); ?> 
							...
							<div class="formitm">
								...
								<div class="ipt">
									<select class="u-select" name="pid">
										...
										<option value="{$key}" <?php echo ($phpstart); ?> if($key == $doc["pid"]) echo 'selected="selected"'; <?php echo ($phpend); ?> >{$vo}</option>	
										</<?php echo ($volist); ?>>				
									</select>
									<p>所属的上级菜单</p>
								</div>
...
<?php if(is_array($field_list1)): $i = 0; $__LIST__ = $field_list1;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$field): $mod = ($i % 2 );++$i; echo (gen_field2($field)); endforeach; endif; else: echo "" ;endif; ?>
						</div>
<?php
 if(count($field_list2) > 0){ ?>
						<div class="tab tab_2">
<?php if(is_array($field_list2)): $i = 0; $__LIST__ = $field_list2;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$field): $mod = ($i % 2 );++$i; echo (gen_field2($field)); endforeach; endif; else: echo "" ;endif; ?>
						</div>
<?php
 } ?>
