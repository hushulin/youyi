...
<body onselectstart="return false">
	...
	<div id="main" class="g-main">
		...
		<div class="row">
			<div class="left">
				<a class="u-btn" href="<?php echo U('/Admin/Collection/add');?>?rurl=<?php echo base64_encode($rurl);?>">新 增</a>
			</div>
		...
		<div class="row">
			<form id="main_form" action="<?php echo U('/Collection/maction');?>" method="get">
				<table class="m-table">
					<thead>
						<tr>
					...
					<tbody>
						<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
							...
							<td><?php if(isset($field_goods_id)){echo $field_goods_id[$vo["goods_id"]];}else{echo $vo["goods_id"];}?></td><td><?php if(isset($field_user_id)){echo $field_user_id[$vo["user_id"]];}else{echo $vo["user_id"];}?></td>							
							<td>
								<a href="<?php echo U('/Admin/Collection/edit/id/'.$vo['id']);?>?rurl=<?php echo base64_encode($rurl);?>" title="">编辑</a>
								<a href="<?php echo U('/Admin/Collection/del/id/'.$vo['id']);?>?rurl=<?php echo base64_encode($rurl);?>" title="">删除</a>
							</td>
