...
<body onselectstart="return false">
	...
	<div id="main" class="g-main">
		...
		<div class="row">
<?php  $rurl = base64_decode($_GET["rurl"]); $rurl = empty($rurl)?U("/Admin/Worth"):$rurl; ?>			<a class="u-btn" href="<?php echo ($rurl); ?>">返回列表</a>
		</div>
		<div class="row">
				...
				<form name="m-form" method="post" enctype="multipart/form-data">
					<fieldset>
						<div class="tab tab_1">
							...
							<div class="formitm">
									<label class="lab">值不值:</label>
									<div class="ipt">
										<select class="u-select" name="is_worth"><?php if(is_array($field_is_worth)): $i = 0; $__LIST__ = $field_is_worth;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($key); ?>" <?php if($key == $doc["is_worth"]) echo 'selected="selected"'; ?> ><?php echo ($vo); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
</select>
										<p></p>
									</div>
								</div>
