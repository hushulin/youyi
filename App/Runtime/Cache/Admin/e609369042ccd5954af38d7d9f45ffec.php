...
<body onselectstart="return false">
	...
	<div id="main" class="g-main">
		...
		<div class="row">
			<div class="left">
				<a class="u-btn" href="<?php echo U('/Admin/Cargoodscate/add');?>?rurl=<?php echo base64_encode($rurl);?>">新 增</a>
			</div>
		...
		<div class="row">
			<form id="main_form" action="<?php echo U('/Cargoodscate/maction');?>" method="get">
				<table class="m-table">
					<thead>
						<tr>
					...
					<tbody>
						<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
							...
							<td><?php if(isset($field_title)){echo $field_title[$vo["title"]];}else{echo $vo["title"];}?></td><td><?php if(isset($field_sort)){echo $field_sort[$vo["sort"]];}else{echo $vo["sort"];}?></td>							
							<td>
								<a href="<?php echo U('/Admin/Cargoodscate/edit/id/'.$vo['id']);?>?rurl=<?php echo base64_encode($rurl);?>" title="">编辑</a>
								<a href="<?php echo U('/Admin/Cargoodscate/del/id/'.$vo['id']);?>?rurl=<?php echo base64_encode($rurl);?>" title="">删除</a>
							</td>
